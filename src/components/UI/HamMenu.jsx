...
export default function HamMenu() {
  const { onClose } = useContext(BurgerMenuContext);
  const { onCategory } = useContext(BurgerMenuContext);
  return (
    <>
      <div>
        <div className="bg-gray-500 absolute z-40 h-full opacity-50 w-full"></div>
        <div className="hamMenu h-full w-64 md:w-2/4 bg-white absolute z-50 left-0 opacity-100">
          <img onClick={onClose} src={hamMenu} className="mt-14 mr-10" alt="" />
